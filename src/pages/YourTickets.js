    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const resU = await axios.get('http://localhost:9999/users');
    ...
    return (
        userLog ? (
            <Container fluid className="p-4 text-white bg-dark">
                <Container>
                    <h2 onClick={() => { console.log(showings); }}>Welcome back <span className="text-success">{user.firstName.toUpperCase()}</span>, how are you today?</h2>
                    <Row>
                        <Col md={3}>
